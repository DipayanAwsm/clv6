...
|---|---|---|---|
| `CLV_PROJECT_ROOT` | Recommended | `..` | Root path for data/reports resolution |
...
```bash
cd backend
PYTHONPATH=. python -m training.run_pipeline --input-csv ../data/raw/<new_file>.csv --high-value-quantile 0.8
```
...
- [ ] `reports/metrics/` and `reports/figures/` updated
...
2. Restore matching `reports/metrics/model_metrics.json` metadata snapshot.
...
- Keep sample CSV ready in `data/sample_input/upload_sample.csv`
